...
$.fn.select = function(callback) {
    ...
    var optionHtml = $select.find(".option.selected").html();
    $select.wrapInner('<div class="options"/>');
    var $options = $select.find(".options");
    $select.append('<input type="hidden" name="'+name+'" value="'+value+'"/>');
    var $input = $select.find("input");
    $select.prepend('<div class="option_selected">'+optionHtml+'</div>');
